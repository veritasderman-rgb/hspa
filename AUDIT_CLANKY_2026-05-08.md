...
### 2026-05-16 · `clanek-vydaje-zdravotnictvi.html` — **kaskádová revize finančního clusteru (Priorita 2: konkrétní čísla s riskem věcné nepřesnosti); sjednocení s ověřenými sourozeneckými články financovani-sha a platba-z-kapsy**

- **Slug a název článku:** `clanek-vydaje-zdravotnictvi.html` — „8,5 procenta HDP. Co české zdravotnictví stojí, co tomu odpovídá a co přijde" (číslo 24, Financování, datum vydání 7. 5. 2026, dosud bez auditu).
- **Důvod priority:** Třetí článek finančního clusteru, který přímo navazuje na již ověřené sourozenecké články `clanek-financovani-sha.html` (re-anchored 2023, 14,6 % OOP, 8,4 % HDP) a `clanek-platba-z-kapsy.html` (OECD ~18 %, EU ~15 % OOP). Bez revize hrozila numerická a referenční inkonzistence napříč třemi články o stejné věci (HDP, OOP, struktura SHA). Článek dále obsahoval ordinální odkazy „(článek 19/20/21)", které neodpovídají moderní hyperlinkové struktuře (a v navigaci se v průběhu života portálu rozcházejí s pořadovými čísly v dashboardu).
- **Stav před a po:**
  - Před: 0 ověřených výroků formálně označených, 5 generických URL ve zdrojích, 0 stabilních primárních permalinků, 0 anchor cross-linků, 0 visual elementů, 1 figure s 9 hodnotami v zastaralé OECD vrstvě 2024 (Eurostat 2024 estimate place-holder), 0 audit metadat, status `published` true.
  - Po: 16 hodnot ověřených proti primárním zdrojům (ČSÚ ZÚ 2010–2023, Eurostat `hlth_sha`, OECD HAAG 2025, OECD/European Observatory CHP 2025 Czechia), 5 nových stabilních permalinků v sekci Zdroje (Eurostat databáze, OECD Data Explorer SHA, OECD HAAG 2025, ČSÚ NZU, CHP 2025 Czechia), 3 anchor cross-linky (vyhnutelné hospitalizace, ambulantní kontakty, hospitalizujeme nejvíc), 1 review-pending banner (article-databox-disclaimer pattern), 1 audit-comment v HTML head, 1 audit blok v articles.json, status `published: false` + `_review_note`.
- **Co bylo přepsáno / opraveno (důkazové):**
  - **Mezinárodní hodnoty výdajů % HDP (Eurostat `hlth_sha`, 2023):** Německo 12,7 → 11,7 %, Francie 11,9 → 11,5 %, Rakousko 11,4 → 11,2 %, EU agregát 10,4 → 10,0 %. Sjednoceno s auditem `clanek-financovani-sha.html` (2026-05-15) a `clanek-platba-z-kapsy.html` (2026-05-15).
  - **USA výdaje % HDP (OECD, 2023):** 17,0 → 16,7 % (oprava o 0,3 p.b., upřesnění z „přes 17 procent" na „zhruba 16,7 %").
  - **Severské země (Eurostat 2023):** „kolem 11 procent" / „10–11" → „9–11 %" (SE 10,7 / DK 10,5 / FI 9,8 / NO 8,9 — generalizace 10–11 vylučovala Norsko a Finsko, které jsou pod 10).
  - **Česká hodnota:** 8,5 % HDP → 8,4 % HDP (ČSÚ 2023, primární zdroj), s explicitním poznámkou „dashboard HSPA pracuje s odhadem 8,5 % pro 2024" (zachovaný estimace, ale označená jako estimace, ne primární data). Title článku ponechán s 8,5 (dashboardová projekce, ne primární vlna), tělo a figure re-ukotveny na 8,4 (2023, ČSÚ).
  - **Databox přímých plateb pacientů (OOP):** 14 % vs. OECD 20 % → 14,6 % (ČSÚ 2023) vs. EU ~15 % (Eurostat) a OECD ~18 % (OECD HAAG 2025). Tato oprava je kritická: původní text uváděl OECD průměr 20 %, který je proti OECD HAAG 2025 chybný — ČR tedy není „výrazně podprůměrná", jak naznačoval databox; je zhruba na průměru EU a mírně pod průměrem OECD. Sjednoceno s `clanek-platba-z-kapsy.html` audit.
  - **Multimorbidita 65+:** ponecháno se značkou `cascade-flagged` v databoxu, doplněn explicit reference na rozdíl mezi dashboardem (65 %) a CHP 2025 Czechia (52–53 %). Hodnota v textu hedge-ována („proti SHARE / OECD CHP 2025 ... ČR cca 52–53 % vs. EU cca 40–46 %") — formálně neopravena v dashboardu (otevřená otázka pro datovou iteraci, naváže na audit `clanek-nadeje-doziti-zdravi.html` z 14. 5. 2026, kde byla rovnocenná hodnota označena k rekonciliaci).
  - **Ordinální odkazy** „(článek 19)" → anchor `<a href="clanek-vyhnutelne-hospitalizace.html">Vyhnutelné hospitalizace</a>`, „(článek 20)" → anchor `<a href="clanek-ambulantni-kontakty.html">Ambulantní kontakty</a>`, „(článek 21)" → anchor `<a href="clanek-hospitalizujeme-nejvic.html">Hospitalizujeme nejvíc</a>`.
- **Co bylo doplněno do sekce Zdroje:**
  - **ČSÚ — Výsledky zdravotnických účtů ČR v letech 2010–2023** (vydáno 2025) — primární zdroj pro národní hodnoty (stabilní URL).
  - **Eurostat — Health care expenditure (`hlth_sha`)** s konkrétním tabulkovým permalinkem `hlth_sha11_hf` (data browser).
  - **OECD Health Statistics — HEALTH_STAT** s permalinkem na data-explorer (filter SHA).
  - **OECD Health at a Glance 2025** (full edition).
  - **OECD / European Observatory — State of Health in the EU. Country Health Profile 2025: Czechia** (institucionální stabilní URL).
  - **ČSÚ — projekce obyvatelstva 2024–2100** (demografická projekce).
  - **SHA 2011 manual** (specifický OECD permalink místo generického `/health-systems-and-financing.html`).
  - **Eurostat SHA metadata** (`hlth_sha11_esms`).
- **Co bylo zachováno z původního článku (ověřeno proti primárním zdrojům nebo vyhodnoceno jako neauditované hodnotové soudy):**
  - Argumentační rámec o tom, že vyšší výdaje neznamenají automaticky lepší výsledky (úvod, ověřeno proti CHP / OECD HAAG, USA jako protipříklad).
  - Tři tlaky příští dekády (demografie, drahé inovativní léky, personální náklady) — věcně potvrzené trendy v OECD HAAG 2025 a národních strategiích.
  - Mezinárodní inspirace (Německo SHI, Skandinávie daňový systém, Nizozemsko 2006 reforma, Velká Británie NHS) — kontextová policy referenční sekce, hodnoty nejsou audit-relevantní (jsou kvalitativní popisy systémů).
  - Strukturální zaměření na rezervy efektivity (lůžková → ambulantní, cenotvorba léčiv, prevence, administrativa) — návazné na ostatní HSPA články, nemění věcná tvrzení.
  - Reformní otázka v závěru — autorský komentář redakce, hodnotový soud (zachováno bez audit zásahu).
- **Souběžné úpravy v `data/articles.json`:**
  - **`vydaje-zdravotnictvi`**: doplněn `audit` blok (status review-pending, last_reviewed 2026-05-16, primary_sources_added 5, unverified_claims_removed 0, visual_elements_added 1); `published: false` + `_review_note` doplněn; perex re-anchored („8,5 % HDP vs. OECD 9,3" → „podle ČSÚ za 2023 zhruba 8,4 % HDP (dashboard odhad 8,5 % pro 2024) vs. EU 10,0 % a OECD ~9,3 %; léky 1,7 % HDP vs. ~1,5 OECD").
- **Otevřené otázky pro ruční schválení:**
  1. **Dashboard indikátor `multimorbidita_65plus`** má `value: 65` proti `benchmark.oecd: 60`, `benchmark.eu: 62` — nesoulad s OECD CHP 2025 Czechia (cca 52–53 % ČR, EU cca 40–46 %). Vyžaduje datovou iteraci, která sjednotí indikátor s ověřenými hodnotami z auditu `clanek-nadeje-doziti-zdravi.html` (2026-05-14). Do té doby ponecháno s cascade-flag.
  2. **Hodnota indikátoru `vydaje_zdravotnictvi_hdp` v dashboardu** (`value: 8.5`, `year: 2024`) zůstává jako estimate pro 2024 — primární ČSÚ data ale končí rokem 2023 (8,4 %). Doporučení: v navazující datové iteraci ponechat 8,5 jako odhad s flagem `is_estimate: true` a doplnit explicitní 2023 hodnotu (8,4) jako primární referenci do trendu.
  3. **OECD průměr výdajů % HDP** — článek (a dashboard) uvádí 9,3 %. OECD HAAG 2025 publikuje hodnotu ~9,2 % pro 2023; rozdíl je v rámci zaokrouhlení, ale stojí za sjednocení v navazující iteraci s `data/indicators.json` (benchmark.oecd `vydaje_zdravotnictvi_hdp`).
  4. **Vizuální obohacení** — nový vizuál nebyl přidán nad rámec opravy existujícího bar grafu (5+ hodnot opraveno); kandidáti pro budoucí iteraci: časová řada % HDP ČR 2010–2023 (z ČSÚ NZU), nebo struktura ČR vs. OECD/EU v podobě donut grafu HDP složení (HC.1 / HC.5 / HC.6 / ostatní) sjednocujícího databox s ověřenými hodnotami v sourozeneckém `clanek-financovani-sha.html`.
- **Předtím / potom:** 1 figure s 9 hodnotami re-anchored (USA, DE, FR, AT, UK, Skandinávie, EU, OECD, ČR; referenční rok 2024 → 2023) · 1 databox se 4 indikátory aktualizován (vydaje_zdravotnictvi_hdp, vydaje_leky_hdp, platba_z_kapsy_pct, multimorbidita_65plus) · 5 nových stabilních primárních permalinků v sekci Zdroje · 3 ordinální odkazy → anchor odkazy · 1 review-pending banner doplněn · 1 audit komentář v HTML head · 1 audit blok v articles.json · 0 výroků smazáno (článek měl tendenci k generickému formulování, ne k vymýšlení hodnot — proto re-anchoring místo deletion) · status published → review-pending. **0 nových samostatných vizuálů** (vizuální obohacení odloženo do navazující iterace po rekonciliaci `multimorbidita_65plus` v datové vrstvě).
