...
.github/workflows/awareness-weekly.yml + PROMPT_ROUTINE.md (blok H)
```
...
Realizace: blok H „Týdny zdraví" v `PROMPT_ROUTINE.md` (jediná rutina
agenta) + `.github/workflows/awareness-weekly.yml`
...
| 3 | ✅ Týdenní rutina: `scripts/awareness-rotate.js` (deterministický přepínač draft→ready + archivace) + `.github/workflows/awareness-weekly.yml` (po 04:00 UTC) + `PROMPT_ROUTINE.md (blok H)` (agent píše obsah) + testy. Microsite obohacena o kontextový blok (proč záleží / co ovlivňuje / jak na tom je ČR / proč den vznikl a jak ho uctít). Pilotní článek `clanek-plne-kojeni-porodnice.html` publikován. |
| 2 | Naplnění dalších marquee dnů (# 2–31) daty + covers — průběžně přes rutinu dávky 3 |
