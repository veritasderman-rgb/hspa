...
| 3 | ✅ Týdenní rutina: `scripts/awareness-rotate.js` (deterministický přepínač draft→ready + archivace) + `.github/workflows/awareness-weekly.yml` (po 04:00 UTC) + `PROMPT_AWARENESS_ROUTINE.md` (agent píše obsah) + testy. Microsite obohacena o kontextový blok (proč záleží / co ovlivňuje / jak na tom je ČR / proč den vznikl a jak ho uctít). Pilotní článek `clanek-plne-kojeni-porodnice.html` publikován. |
