...
const introParas = spec.intro.paragraphs.map((p, i) =>
  `<p style="margin:0 0 14px;${i === 0 ? SERIF + 'font-size:17px;' : SANS + 'font-size:15px;'}line-height:1.58;color:${i === 0 ? '#1f1a14' : '#3a332a'};">${esc(p)}</p>`
).join('\n        ');

const html = `<!DOCTYPE html>
<html lang="cs">
<head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Skóre zdravotnictví — newsletter</title></head>
...
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#f3ece0;">
  <tr><td align="center" style="padding:28px 12px;">
    <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="width:600px;max-width:600px;background:#fbf8f1;border:1px solid rgba(31,26,20,.14);">

      <tr><td style="padding:26px 34px 18px;border-bottom:2px solid #1f1a14;">
      ...
      <tr><td style="padding:30px 34px 6px;">
        ...
        <h1 style="margin:0 0 14px;${SERIF}font-size:27px;line-height:1.16;color:#1f1a14;font-weight:700;letter-spacing:-.015em;">${esc(spec.intro.heading)}</h1>
        ${introParas}
      </td></tr>

      <tr><td style="padding:14px 34px 0;"><div style="border-top:1px solid rgba(31,26,20,.16);"></div></td></tr>
...
${rest.map((a, i) => listCard(a, i === rest.length - 1)).join('\n')}
      ...
      <tr><td style="padding:24px 34px 30px;border-top:1px solid rgba(31,26,20,.16);">
