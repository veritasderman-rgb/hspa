...
1. Aplikuj akce (${model === 'Sonnet' ? 'u této položky jde jen o doplnění zdrojů / source_note a registr — žádný zásah do prose' : 'včetně případné jediné upřesňující věty z prose_change → audit-status VŽDY review-pending (articles.json audit-status + audit.status + <meta name="article:audit-status">), i když byl článek partial; contradicted → audit-status partial + GitHub issue přes mcp__github__issue_write (label evidence-audit), když nástroj chybí, dej jen flagged a do problems napiš „issue založí hlavní session“'}). Studii, kterou soubor už cituje, nepřidávej podruhé. Nikdy neměň čísla, published, date, number, audit.last_reviewed, generované artefakty.
2. Do data/articles.json (audit.notes) resp. do audit: komentáře článku připoj větu „evidence-audit ${cfg.today}: N tvrzení, verdikty …; viz data/evidence-audit.json“ — N a verdikty přesně podle registru (jen skutečně auditovaná tvrzení, vyřazená administrativní se nepočítají); u indikátoru se poznámka nepíše, karta dostane pole literature.
3. Spočítej hash AŽ PO úpravách: node scripts/evidence-audit-queue.js --hash ${item.path}
4. Zapiš záznam do data/evidence-audit.json (items; existující id nahraď; aktualizuj generated_at na ${cfg.today}T00:00:00Z): id, type, checked_at ${cfg.today}, run_id ${cfg.run_id}, content_hash, tools, models {search: "sonnet", adjudicate: ${r.adjudicated ? '"opus"' : '"none"'}, write: "${model.toLowerCase()}"}, calls, claims, summary (počty verdiktů, klíče supported/partial/contradicted/no_evidence/not_applicable/unclear), actions, followup (co zůstalo neověřeno — vynechaná tvrzení, citace bez DOI, vyčerpaný strop; fronta položku vrátí jako stale; když nic, pole vynech), notes. Hodnoty models jen sonnet|opus (adjudicate vynech, když nebyla).
